...
if ( ! empty( $user_query->results ) ) {
   ...
   if( $wpu->ID == 1 ){ 
 	        echo"<h4 style=\"color:brown\">The default install admin has NOT been imported into phpBB</h4>";
    }
  ...
  if( ($wpu->ID != 1) ){ 
       ...
       $user_exist = $phpbb_conn->get_row("SELECT username FROM ".$phpbb_config_file["table_prefix"]."users WHERE username = '$wpul'");
        ...
        VALUES ('','$phpbb_user_type','2','','0','', '$wpur', '$wpul', '$wpunc', '$wpup', '$time', '$wpue', '$user_email_hash', '', '', '', '', '', '', '0', '0', '0', '0', '0', '0', '0', 'en', 'Europe/Rome', 'D M d, Y g:i a', '1', '0', '', '0', '0', '0', '0', '-3', '0', '0', 't', 'd', 0, 't', 'a', '0', '1', '0', '1', '1', '1', '1', '230271', '', '', '0', '0', '', '', '', '', '', '', '', '0', '0', '0') ON DUPLICATE KEY UPDATE user_password = '$wpup',user_email = '$wpue',user_email_hash = '$user_email_hash'");
...
<h4><span style="color:red">Please Read</span>: do not put so hight value for users to transfer each time. It is set by default to 20 users x time, but you can change the value.<br />Try out: maybe 50, 100 or also 500 or more users to be added x time is ok for your system/server resources.<br />If error come out due to max execution time, it is necessary to adjust to a lower value the number of users to be added x time.<br />Refresh manually from browser: it will "reset the counter" of the transfer procedure.<br /> 
 Repeat the process by setting a lower value for users to be added x time: continue adding users until a <span style="color:green">green message</span> will display that the transfer has been completed.<br />After this, please remember to Fix phpBB values about Total and Newest Members here below.<br />If there is an existent same username on phpBB, his email address and password are overwrite by the email address and password of the transferred WP user. The process exclude both WP and phpBB default install admins. 
...
<input type="submit" name="submit" id="submit" class="button button-primary" value="<?php esc_attr_e($start_or_continue_msg, 'wp_w3all_phpbb');?>">
</p></form></div>
<hr /><hr />

  ...
  $w3all_uap[0] = sanitize_user( $w3all_uap[0], $strict = false );
   ...
   if ( empty($wp_user_data) ){ // this user need to be added also 
   	 ...
   	 if( !empty($userdata) && $userdata->user_id > 2 ){
         ...
         // check for 3 more of these on class.wp.w3all-phpbb.php in case you need to add something or remove
              
          $pattern = '/\'/';
          preg_match($pattern, $w3all_uap[0], $matches);
          
          if($matches){
	         echo '<p style="padding:30px;background-color:#fff;font-size:1.3em">Sorry, your <strong>username '.$w3all_uap[0].' on our forum contain characters not allowed on this CMS system</strong>, you can\'t be added or login on this site part, and you\'ll see this message, if logged in on forums as <b>'.$phpbb_user_session[0]->username.'</b>. Please return back and contact the administrator reporting about this error issue. Thank you <input type="button" value="Go Back" onclick="history.back(-1)" /></p>';
         return;
          	//exit;
          }
